...
(defvar outline-cycle-min-level 1
  "The minimum header level present in this file.  OVERVIEW mode will
call hide-sublevels with this value.  Typically modified when outline-regexp
has a minimum length greater than 1, e.g. ';;;+', and you haven't installed
an appropriate outline-level handler.  Also useful to set the depth of
the OVERVIEW tree.")
(make-variable-buffer-local 'outline-cycle-min-level)
...
- When point is not at the beginning of a headline, execute
  ...
  (cond
   ...
   (t
	...
	(hide-sublevels outline-cycle-min-level)
	(message "OVERVIEW")
