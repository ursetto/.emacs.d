...
(defvar sepia-root (expand-file-name "~/src/perl/sepia")
...
"Install Sepia bindings in the current local keymap."
  ...
  (let ((map (or map (current-local-map))))
    ...
    (define-key map "\C-c\C-r" 'sepia-eval-defun-region)
  ...
  (unless noinit
    ...
    (setq perl-process
      ...
      (with-current-buffer "*perl-interaction*"
        (sepia-comint-setup))
      (accept-process-output perl-process 0 1)
...
;; Doesn't even use 'beg'.
(defun sepia-end-of-defun (&optional where)
  (interactive "d")
  (let ((here (point)))
    (beginning-of-defun)
    (let ((beg (point))  ;; unused
...
"Do the moral equivalent of perl -ne on region (i.e. evaluate an
expression on each line of region).  With prefix arg, replace the
region with the result."
  (interactive "MExpression:\nr\nP")
  (my-perl-frob-region
...
rebuild its Xrefs."
  ...
  (save-excursion
    (let* ((pt (point))
           ...
           (beg (progn (sepia-beginning-of-defun pt) (point))))
      (goto-char beg)
...
(defun sepia-eval-defun-region (beg end)
  "Re-evaluate the current sub in the appropriate package, and
rebuild its Xrefs."
  (interactive "r")
  (save-excursion
	...
	(let* ((sub (match-string 1))
	  ...
	  (sepia-eval (if sepia-eval-defun-include-decls
			  ...
			  body))
	  (xref-redefined sub sepia-eval-package)
	  (message "Defined %s in package %s" sub sepia-eval-package))))))
...
the only function that requires EPL (the rest can use Pmacs)."
 ...
 (if sepia-eval-file (concat "$Sepia::Xref::file = \"" sepia-eval-file "\";")
     "")
 (if sepia-eval-line (format "$Sepia::Xref::line = %d;\n#line %d\n"
                             sepia-eval-line sepia-eval-line)
     "")
 (if discard
     (concat string "; '' }\n")
     (concat
